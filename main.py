...
@main.command()
@click.option('-model', '--model', help='Path to Model trained on SYNTH', required=False)
...
def pre_process(dataset):
	...
	if dataset.lower() == 'ic13':
		...
		if \
			...
			icdar2015_test(
				image_path=config.dataset_pre_process['ic15']['test']['target_folder_path'],
				base_path=config.dataset_pre_process['ic15']['test']['target_json_path'],
				output_path='/home/vedant/tmp/CRAFT-Remade/dataset/ic15/test_gt.json'
			)

			icdar2015_train(
				image_path=config.dataset_pre_process['ic15']['train']['target_folder_path'],
				base_path=config.dataset_pre_process['ic15']['train']['target_json_path'],
				output_path='/home/vedant/tmp/CRAFT-Remade/dataset/ic15/train_gt.json'
			)
